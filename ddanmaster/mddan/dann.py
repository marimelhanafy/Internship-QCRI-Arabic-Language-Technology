        ...
        for nunits in arch:
            ...
            if isinstance(nunits, int):
                ...
                if activations == 'prelu':
                    network += [layers.Activation(activations.relu,(self.layers[-1]))]
                elif activations == 'leakyrelu':
                    network += [layers.Activation(activations.LeakyReLU,(self.layers[-1]))]
                elif activations == 'elu':
                    network += [layers.Activation(activations.ELU,(self.layers[-1]))]
                else:
